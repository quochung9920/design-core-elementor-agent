...
class Design_Core_Elementor_Figma_Design_IR_Adapter {
    ...
    private function layout(array $figma){
        ...
        if(isset($box['width'])&&is_numeric($box['width'])&&(float)$box['width']>0){$layout['max_width']=array('value'=>(float)$box['width'],'unit'=>'px');if('FIXED'===$horizontal&&(float)$box['width']<=320){$layout['width']=array('value'=>(float)$box['width'],'unit'=>'px');}}
        if(isset($box['height'])&&is_numeric($box['height'])&&(float)$box['height']>0&&'FIXED'===$vertical){$layout['min_height']=array('value'=>(float)$box['height'],'unit'=>'px');}
