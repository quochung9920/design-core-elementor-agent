...
class Design_Core_Elementor_Design_Memory_Store {
    ...
    public function lessons() { return $this->read( self::LESSONS_OPTION, 'lessons' ); }
    public function incidents() { return $this->read( self::INCIDENTS_OPTION, 'incidents' ); }
    ...
    public function upsert_lesson( array $lesson ) {
        ...
        $records = $this->lessons(); $merged = false;
        foreach ( $records as $index => $existing ) {
            ...
            $records[ $index ] = array_replace_recursive( $existing, $lesson ); $merged = true; break;
        }
    ...
    private function write( $option, $key, array $records ) { update_option( $option, array( 'version' => self::VERSION, $key => array_values( $records ), 'updated_at' => gmdate( 'c' ) ), false ); }
    private function cap( array $records, $limit ) {
        usort( $records, static fn( $a, $b ) => strcmp( (string) ( $b['updated_at'] ?? $b['created_at'] ?? '' ), (string) ( $a['updated_at'] ?? $a['created_at'] ?? '' ) ) );
        return array_slice( $records, 0, max( 1, (int) $limit ) );
    }
    private function lesson_id( array $lesson ) { return 'lesson_' . substr( hash( 'sha256', implode( '|', array( (string) $lesson['signature'], (string) $lesson['scope'], (string) $lesson['source_type'] ) ) ), 0, 20 ); }
    ...
    private function scope( $value ) { $value = sanitize_key( (string) $value ); return in_array( $value, array( 'global', 'project', 'source' ), true ) ? $value : 'global'; }
    private function sanitize_record( array $record ) {
        if ( class_exists( 'Design_Core_Elementor_Change_Ledger' ) && method_exists( 'Design_Core_Elementor_Change_Ledger', 'transport_safe' ) ) {
            $safe = Design_Core_Elementor_Change_Ledger::transport_safe( $record ); return is_array( $safe ) ? $safe : array();
        }
    ...
    private function bounded( $value, $depth = 0 ) {
        ...
        if ( is_string( $value ) ) { return function_exists( 'mb_substr' ) ? mb_substr( $value, 0, 4000 ) : substr( $value, 0, 4000 ); }
