...
# Fast mode:  ./scripts/ci-local.sh --fast
set -Eeuo pipefail
FAST=0
...
set +u
if [[ -f /home/dev/wordpress-dev/env.sh ]]; then source /home/dev/wordpress-dev/env.sh >/dev/null 2>&1 || true; fi
set -u
...
stage "PHP"; if [[ -x /home/dev/wordpress-dev/runtime/usr/bin/php8.5 ]]; then PHP_BIN=/home/dev/wordpress-dev/runtime/usr/bin/php8.5; ok; elif PHP_REAL="$(type -P php 2>/dev/null)" && [[ -n "$PHP_REAL" ]]; then PHP_BIN="$PHP_REAL"; ok; elif command -v php >/dev/null 2>&1; then PHP_BIN=php; ok; else blk "no php in PATH" "php"; fi
stage "Node"; if command -v node >/dev/null 2>&1; then ok; else blk "node missing" "node"; fi
stage "npm"; if command -v npm >/dev/null 2>&1; then ok; else blk "npm missing" "npm"; fi
...
if command -v docker >/dev/null 2>&1; then
  for V in 8.1 8.2 8.3; do
    stage "PHP $V"
    if docker build -q --build-arg "PHP_VERSION=$V" -f docker/ci/php/Dockerfile . >"$ARTIFACTS/php-$V-image.txt" 2>&1 && docker run --rm -v "$ROOT:/repo:ro" "$(cat "$ARTIFACTS/php-$V-image.txt")" php -l design-core-elementor.php >"$ARTIFACTS/php-$V-lint.txt" 2>&1; then ok; else bad "see $ARTIFACTS/php-$V-*.txt" "php-$V"; fi
  done
else
...
say ""; say "Contracts"
CONTRACTS="architecture registry native-fidelity strict-native responsive-compiler design-intelligence design-brain rc19-intelligence rc20-fidelity reference-integration fixed-width-governance figma-fidelity pawcare-benchmark design-memory figma-vector-assets"
for T in $CONTRACTS; do
  stage "$T"
  if [[ "$T" == "figma-fidelity" ]]; then
    if "$PHP_BIN" tests/figma-fidelity/run.php >"$ARTIFACTS/contract-$T.txt" 2>&1 && "$PHP_BIN" tests/figma-fidelity/adapter.php >>"$ARTIFACTS/contract-$T.txt" 2>&1 && "$PHP_BIN" tests/figma-fidelity/geometry.php >>"$ARTIFACTS/contract-$T.txt" 2>&1; then ok; else bad "see $ARTIFACTS/contract-$T.txt" "contract-$T"; fi
...
say ""; say "=================================================="
if [[ $FAIL == 0 && $BLOCKED == 0 ]]; then say " LOCAL CI PASS ($PASS stages)"; say "=================================================="; exit 0; else say " LOCAL CI FAIL — pass=$PASS fail=$FAIL blocked=$BLOCKED"; say " Failed: ${FAILED_STAGES[*]}"; say "=================================================="; exit 1; fi
